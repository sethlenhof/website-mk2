      ...
      >
        {selected ? (
          <div>
            ...
            <div>
              <h5 className="text-lg font-semibold text-white">Role Progression</h5>
                ...
                {selected.roles.map((role, index) => (
                  <div key={`${role.title}-${role.start}`} className="relative pl-8">
                    ...
                    {index !== selected.roles.length - 1 && (
                      <div className="absolute left-[11px] top-1/2 h-[calc(100%+1.5rem)] w-px bg-white/10" />
                    )}
                    ...
                    <div
                      className={`absolute left-0 top-1/2 -translate-y-1/2 flex h-6 w-6 items-center justify-center rounded-full border ${
                        role.current
                          ? "h-7 w-7 border-emerald-400/30 bg-emerald-400/20"
                          : "h-6 w-6 border-white/15 bg-white/5"
                      }`}
                    >
                      <div
                        className={`rounded-full ${
                          role.current ? "h-3 w-3 bg-emerald-300" : "h-2.5 w-2.5 bg-white/40"
                        }`}
                      />
                    </div>
